...
export default function App() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (allEvents.length > 0) {
        ...
        setMapId(firstMap);
        setMatchId(allEvents[0].match_id);
        setPlayerId('All');
  ...
  const activePlayersCount = useMemo(() => {
    return new Set(filteredEvents.map(e => e.user_id)).size;
  }, [filteredEvents]);

  const { kills, deaths } = useMemo(() => {
    let k = 0; let d = 0;
    filteredEvents.forEach(e => {
      if (e.event === 'kill') k++;
      if (e.event === 'death' || e.event === 'storm_death') d++;
    });
    return { kills: k, deaths: d };
  }, [filteredEvents]);
    ...
    <div className="flex h-screen w-screen bg-gray-50 text-gray-900 overflow-hidden font-sans">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex-1 p-6 flex flex-col relative min-h-0">
          {/* Top Bar / Upload */}
          <div className="absolute top-8 left-8 z-10 flex flex-wrap gap-3">
            <div className="px-4 py-2 bg-white/90 backdrop-blur-sm border border-gray-200 rounded-lg shadow-md flex items-center gap-2">
              <Users className="w-4 h-4 text-indigo-500" />
              <span className="text-sm font-medium">Active Players: {activePlayersCount}</span>
            </div>
            <label className="flex items-center gap-2 px-4 py-2 bg-white/90 backdrop-blur-sm border border-gray-200 hover:bg-gray-100 rounded-lg cursor-pointer transition-colors shadow-md">
            ...
            {filteredEvents.length > 0 ? (
              ...
              <div className="text-gray-500 flex flex-col items-center gap-4">
                <div className="w-16 h-16 rounded-full bg-gray-200/50 flex items-center justify-center">
                  <MapCanvas className="w-8 h-8 opacity-50" events={[]} mapId={mapId} layers={layers} customMapImages={customMapImages} />
                </div>
